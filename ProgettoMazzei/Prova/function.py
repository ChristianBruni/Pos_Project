...
def viterbi(words, tags, tm, em):
    ...
    possible_tags = [tag[0] for tag in tags if tag[0] != 'S0']
    n_tags = len(possible_tags)
    n_words = len(words)

    # Inizializza trellis e backpointers
    trellis = np.zeros((n_tags, n_words))
    backpointers = np.zeros((n_words, n_tags), dtype=int)

    # Smoothing
    epsilon = 1e-8

    # Passo 1: Inizializzazione
    first_word = words[0] if n_words > 0 else None
    for i, tag in enumerate(possible_tags):
        # Probabilità iniziale (transizione da S0)
        trans_prob = tm.loc[tag, 'S0'] if 'S0' in tm.columns else epsilon

        # Probabilità emissione (gestione parole sconosciute)
        emit_prob = em.loc[tag, first_word] if first_word in em.columns else epsilon

        trellis[i, 0] = np.log(trans_prob + epsilon) + np.log(emit_prob + epsilon)

    # Passo 2: Riempimento del trellis
    for t in range(1, n_words):
        current_word = words[t]
        for i, current_tag in enumerate(possible_tags):
            max_prob = -np.inf
            best_prev = 0

            # Emissione (gestione parole sconosciute)
            emit_prob = em.loc[current_tag, current_word] if current_word in em.columns else epsilon

            for j, prev_tag in enumerate(possible_tags):
                trans_prob = tm.loc[current_tag, prev_tag] if prev_tag in tm.columns else epsilon
                prob = trellis[j, t - 1] + np.log(trans_prob + epsilon) + np.log(emit_prob + epsilon)
    ...
    best_path = []
    if n_words > 0:
        best_last = np.argmax(trellis[:, -1])
        best_path.append(possible_tags[best_last])

        for t in range(n_words - 1, 0, -1):
            best_last = backpointers[t, best_last]
            best_path.insert(0, possible_tags[best_last])

    return list(zip(words, map(str, best_path)))
...
def calc_accuracy(predict_seq, true_seq):
    predict = [str(tag) for sentence in predict_seq for _, tag in
               sentence]  # estrae dalla sequenza predetta tutti i tag e li mette in un vettore

    true = [tag for s in true_seq for tag in s.item().split() if
            tag != 'S0']  # estrae dalla vera sequenza tutti i tag e li mette in un vettore
...
def baseline_tagger(words, tags, em):
    ...
    for key, word in enumerate(words):  # ciclo su tutte le parole da taggare
        ...
        state_word = 'NOUN' if max_prob == 1e-6 else tags[p.index(max_prob)]
...
def evaluate_baseline(train, train_tags, test_tags, test_words):
    ...
    predicted_tags = []
    for sentence in test_words:
        tagged_seq = baseline_tagger(sentence.split(), count_tags, em)
        print(tagged_seq)
